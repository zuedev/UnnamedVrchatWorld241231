...
//                }
		...
		public void ComputeGrass()
		{
			...
			float detailFactor = area2D.currentTerrain.terrainData.detailScatterMode == DetailScatterMode.CoverageMode ? 255f : 16f;
#else
			float detailFactor = 16f;
#endif
			...
			for (int i = 0; i < grassLength; ++i)
			{
				...
				for (int y = 0; y < resolution; ++y)
				{
					for (int x = 0; x < resolution; ++x)
					{
						...
						grass[y, x] = (int)(((float)bytesArray[index].bytes[byteIndex] / 255.0f) * detailFactor);
					}
				}
		...
		public void ComputeTree()
		{
			...
			area2D.currentTerrain.terrainData.RefreshPrototypes();
